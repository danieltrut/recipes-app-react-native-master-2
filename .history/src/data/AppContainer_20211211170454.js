...
class ApiContainer extends Component {
    ...
    renderCategory = ({ data }) => (
        <TouchableHighlight underlayColor="rgba(73,182,77,0.2)" onPress={() => onPressCategory(data.item)}>
            <View style={styles.categoriesItemContainer}>
                <Image style={styles.categoriesPhoto} source={{ uri: data.item.photo_url }} />
                <Text style={styles.categoriesName}>{data.item.name}</Text>
                <Text style={styles.categoriesInfo}>{getNumberOfRecipes(data.item.id)} recipes</Text>
            </View>
        </TouchableHighlight>
    );
